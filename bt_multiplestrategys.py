# As example: take class "CrossStrategy" from line 86, put in into line 1400 into "cerebro.addstrategy(...)", run the script
...
if __name__ == '__main__':
    ...
    # Create a feed for 1 min data
    data_1min = MyPandasData(dataname=my_data_frame)  

    # Create a resampled feed for 7 min 
    data_7min = MyPandasData(
        dataname=my_data_frame,
    ...
    # BTCUSDT_2023-3-06_1min.csv
    # BTCUSDT_1min.csv
    # BTCUSDT_3min.csv
    # DOGEUSDT_1min.csv
    # ETHUSDT_1min.csv
    # my_data_frame = pd.read_csv('../candles/BTCUSDT_3min.csv', index_col=0, parse_dates=True)
    
    data = MyPandasData(dataname=my_data_frame)
    
    # data = MyPandasData(dataname=pd.read_csv('../candles/BTCUSDT_1h.csv', index_col=0, parse_dates=True)) # create a Backtrader feed from the Pandas DataFrame

    cerebro = bt.Cerebro()
    # Add both to cerebro 
    cerebro.adddata(data_1min, 'btc_1min')
